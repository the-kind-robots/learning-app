...
- A coordinator running in the background has no third option. It does not edit, and the harness refuses a background session's writes to the shared checkout — so every delegated edit lands in a worktree. Launch the agent with `isolation: "worktree"`. The refusal also offers to have the coordinator enter a worktree first: do not take that way out, the pin is inherited and the order below stops working (#346). Neither, and the agent stops on its first write having done nothing.
...
- Where to work is a dependency test, not a topic match. Name the shared thing the work needs — a live CouchDB, nginx routing, a migration over real data — and it goes to the main checkout. Cannot name one? Worktree. Subsystems are examples, never the test: #351 was sent to the main checkout for touching the dictionary and needed nothing shared.
