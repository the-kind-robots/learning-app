...
## Purpose
Define when a new service worker takes over: it stays waiting until the user asks for it, every page that was under a controller reloads once when the controller changes, and the registration checks for a new worker when the app comes back into view.
## Requirements
