...
- **THEN** the result includes documents with no `collection-id`, with `collection-id == T1`, `collection-id == T2`, etc.
...
### Requirement: Backend endpoint accepts optional context parameter
The system SHALL forward an optional `context` query parameter from `/api/examples` to the AI prompt generator. If the parameter is absent or blank, no collection context is injected.
...
#### Scenario: No injection without context
