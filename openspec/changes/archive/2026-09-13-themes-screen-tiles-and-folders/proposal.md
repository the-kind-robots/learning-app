...
- Collections render as small tiles in a masonry — name, count, colour accent — two columns on a phone, four from 700 px, one alphabetical sequence read down each column (locale-aware, case-insensitive). «Всё подряд» is a tile pinned first.
- A `/` in a name makes a folder: the part before the first `/` is the folder key, the rest is a child row on one folder tile. The header is the collection named exactly as the key; its count is the union of its own words and its children's. With no such collection the header is a plain label carrying the children's union count: not a tap target, and the parent is created through «+» like any collection.
...
- `collections-navigation`: tiles in a masonry replace the cards; folders by `/`, whose header is the parent collection or, with no such document, a plain label; floating «+» replaces the create card; the screen reads no vocab or review documents.
