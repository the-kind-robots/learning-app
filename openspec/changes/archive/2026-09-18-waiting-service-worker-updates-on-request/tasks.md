...
- [x] 2.1 `service-worker` namespace: register, detect a waiting worker (`registration.waiting`, `updatefound` → `installed` with a controller), offer it — saves `:pwa/update-waiting?` in every build, never activates unasked; `controllerchange` reloads once for a page that started controlled; `visibilitychange` → `registration.update()`; `:effect/activate-waiting-worker`
- [x] 2.2 Presenter `:show-update?`; the shell shows «Обновить» in the actions row; CSS for the text button
- [x] 2.3 The trace export dispatches `:effect/export-trace` from the actions row; the red D is a span in the word mark
...
- [x] 3.2 `docs/dev/mobile-pwa-testing.md`: the update control and the trace export
