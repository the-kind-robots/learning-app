...
- Force a reload from the page in a development build.
...
- **A tap each, never a hold.** The forced reload and the trace export are two controls rather than a tap and a hold on one: the reload is asked for constantly and the export rarely, and telling a hold from a tap would cost both of them a recogniser — a timer, a movement threshold and a flag the click has to read. Which control each one is, the spec says.
...
- [A reload mid-lesson] → a page only reloads after the user tapped «Обновить» or asked for the forced reload; nothing reloads unasked, in any build.
