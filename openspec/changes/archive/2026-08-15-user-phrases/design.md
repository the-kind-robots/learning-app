...
5. **Automatic mode detection, no manual toggle.** Space in trimmed input → phrase, except article+word (der/die/das/ein/eine), `sich`+word, or input matching a non-phrase dictionary lemma from already-fetched completions; picking a suggestion decides by its pos (multi-word pos=phrase → phrase). No manual control at all: a chip was built as an override and dropped on review. It duplicated what the legend and the label already say, and the only direction it offered — phrase to word — is the one that splits a sentence translation on its commas. Both fields are always auto-growing textareas (rows=1 compact) so a mode flip never remounts inputs or loses text. Mode flip must clear open suggestions and guard stale async completion responses. Alternative — explicit segment toggle — rejected by owner as tiresome.
...
8. **A review per graded attempt, for phrases as for words.** A per-lesson cap was built first (`:reviewed-trial-ids`, first attempt decides) to stop a hard phrase writing four lapses in one lesson, and dropped on review: it made a correction within the lesson count for nothing, and it applied to phrases alone. Close lapses make the log fuller, not noisier; how the scheduler weighs them is a question for the SRS settings, not for the trial loop.
...
- [Space heuristic misfires on multi-word lemmas not in completions yet] → article/`sich` exceptions + suggestion-pos override; typing the noun with its article settles it. A word saved as a phrase costs one generated example and nothing else, and the kind is a field, so an editor can correct it later.
