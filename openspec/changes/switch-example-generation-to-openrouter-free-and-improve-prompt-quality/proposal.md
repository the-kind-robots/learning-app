...
- move example generation onto OpenRouter with a free model path
...
- Replace direct `api.openai.com` example generation calls with OpenRouter-compatible chat completions.
- Introduce a configurable free-model strategy, starting with a pinned OpenRouter free model and keeping router-level free inference available as an override.
...
- `examples`: Example generation transport, model selection, and prompting become configurable around OpenRouter free inference rather than a single direct OpenAI model.
