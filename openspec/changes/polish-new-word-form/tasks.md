...
- [ ] 1.2 Verify in the browser: typing a prefix, ArrowDown to a suggestion, then Enter, fills the input with the lemma and focuses the translation field (no spurious form submit)
...
- [ ] 2.3 Verify in the browser: open dropdown, click outside → dropdown closes; open dropdown, click a suggestion → suggestion is picked (no race)
...
- [ ] 3.6 Browser verify: typing "mutter" in the form returns only Mutter-related lemmas
...
- [ ] 4.4 Verify in the browser: Tab through the home page form — every focusable element shows a visible ring
...
- [ ] 5.3 OpenSpec archive applied to `openspec/changes/archive/<date>-polish-new-word-form/`
- [ ] 5.4 Canonical specs updated for `home-add-form-focus` and `dictionary-storage`
