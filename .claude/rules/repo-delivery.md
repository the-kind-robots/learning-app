...
The board is the organization project `Learning app` (`the-kind-robots`, number 11). Priority is **Urgent/High/Medium/Low** on the org's native issue field, not a project field — the script sets it with `setIssueFieldValue`, and the retired `Blocker/Critical/Major/Minor/Trivial` are rejected by name.
