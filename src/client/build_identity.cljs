(ns build-identity
  "Which build the page is running: the short commit of the checkout it was
   ...
   `goog-define` is what makes that possible: it declares a compile-time
   constant with a default, which the Closure compiler replaces from
   `:closure-defines` — where the hook writes the stamp — and folds into
   every reader. Nothing set, and the default stands.
