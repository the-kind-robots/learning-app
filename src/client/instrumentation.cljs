...
(defn render!
  "Runs one render through the metrics and the trace: counted, and recorded
   ...
   inside or after the re-render that may have replaced its node. Counted
   here, on the render path itself, not from a store watch: with unbatched
   saves one dispatch may notify the store several times while rendering
   once, so notifications do not measure renders."
  [render-fn state]
