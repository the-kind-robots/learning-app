...
(nxr/register-effect! :effect/load-lesson
  (fn ^:async load-lesson [{:keys [dispatch]} _]
    (try
      (let [{:keys [lesson-state error]} (await (lesson/restart! (dbs/dbs)))]
...
(nxr/register-effect! :effect/check-answer
  (fn ^:async check-answer [{:keys [dispatch]} _ answer]
    (try
      (let [{:keys [lesson-state]} (await (lesson/check-answer! (dbs/dbs) answer))]
        (when lesson-state
...
(nxr/register-effect! :effect/next-trial
  (fn ^:async next-trial [{:keys [dispatch]} _]
    (try
      (let [{:keys [lesson-state]} (await (lesson/advance! (dbs/dbs)))]
        (when lesson-state
          (dispatch [[:action/update-lesson lesson-state]])))
      (catch js/Error err
...
(nxr/register-effect! :effect/end-lesson
  (fn ^:async end-lesson [_ _]
    (try
      (await (lesson/finish! (dbs/dbs)))
...
(nxr/register-effect! :effect/open-token-info
  (fn ^:async open-token-info [{:keys [dispatch]} _ {:keys [dictionary-form translation]}]
    (try
      (let [td (await (lesson/token-info (dbs/dbs) dictionary-form translation))]
...
(nxr/register-effect! :effect/add-token
  (fn ^:async add-token [{:keys [dispatch]} _ {:keys [dictionary-form translation]}]
    (try
      (await (lesson/add-word-from-structure! (dbs/dbs) dictionary-form translation))
