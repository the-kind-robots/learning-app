...
(defn- target-attrs
  "What every tappable thing carries: its id for the gesture tracking, its
   tap, and a long press where there is something to delete. The element it
   goes on says what it is: `role` on the two that are divs, `<button>` for
   the row, which sits in a list."
...
(defn- folder-row
  "A list item holding the row's own button. The `<ul>` keeps its items and
   the tap target is an element that already is one, so nothing has to claim
  ...
  [:li.tile__row
   ...
   (close-button row)
   [:button.tile__row-target
    (assoc (target-attrs row) :type "button")
